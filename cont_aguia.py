...
def verificar_consistencia_contabil(df):
    ...
    # 1a. Equação patrimonial: Ativo = Passivo + PL + Resultado do Exercício
    # Como o sistema exporta tudo positivo, a diferença é a subtração.
    passivo_total_simulado = passivo + resultado_dre
    diferenca_bp = ativo - passivo_total_simulado
    
    if abs(diferenca_bp) > 0.05:
        criticas.append(
            f"❌ **Desequilíbrio Patrimonial:** Ativo ({formatar_num_br(ativo)}) ≠ "
            f"Passivo+PL+Resultado ({formatar_num_br(passivo_total_simulado)}). "
